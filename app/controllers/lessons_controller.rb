class LessonsController < ApplicationController
  ...
  def index
    if params[:teacher].present?
      @lessons = Lesson.where({ user: current_user, teacher: params[:teacher] }).reverse_chronologically
      ...
      @lessons = Lesson.where({ user: current_user, room: params[:room] }).reverse_chronologically
      ...
      @search = Lesson.where({ user: current_user }).reverse_chronologically.ransack(params[:q])
